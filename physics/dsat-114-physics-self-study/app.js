...
const STORE = "dsat114physics_quiz_v2";
...
function qScore(q) { return Number(q.score || QUIZ_META.perScore || 1); }
function isMulti(q) { return q.t === "multi" || Array.isArray(q.a); }
function isOpen(q) { return q.t === "open"; }
function normArr(v) { return Array.isArray(v) ? v.slice().map(Number).sort((a,b)=>a-b) : (v === undefined ? [] : [Number(v)]); }
function isCorrect(q, ans) {
  if (isOpen(q)) return ans && ans.self === true;
  if (isMulti(q)) { const a = normArr(ans), b = normArr(q.a); return a.length === b.length && a.every((x,i)=>x===b[i]); }
  return ans === q.a;
}
function answerLabel(q, ans) {
  if (ans === undefined || ans === null) return "未作答";
  if (isOpen(q)) return ans.self === true ? "自評正確" : (ans.text ? "已作答・待加強" : "未作答");
...
function renderQuestion() {
  ...
  const answered = s.ids.filter(qid => isAnswered(s.answers[qid])).length;

  const curAns = s.answers[id];
  const opts = isOpen(q) ? `<div class="expl"><b>非選擇題：</b>先在紙上或下方框中寫出你的解題過程，再依官方參考答案自評。</div>
      <textarea id="openAns" class="open-input" placeholder="可簡記自己的作答重點（只存在本機）" oninput="setOpenText(this.value)">${(curAns&&curAns.text)||""}</textarea>
      <div class="row" style="margin-top:10px"><button class="btn ${curAns&&curAns.self===true?'primary':''}" onclick="markOpen(true)">我自評正確</button><button class="btn ${curAns&&curAns.self===false?'bad':''}" onclick="markOpen(false)">需要再練</button></div>` : q.o.map((txt, i) => {
    const arr = normArr(curAns);
    const sel = arr.includes(i) ? " sel" : "";
  ...
  const nav = s.ids.map((qid, i) => {
    let c = "";
    if (i === s.idx) c += " cur";
    if (isAnswered(s.answers[qid])) c += " done";
...
window.pick = function (i) {
  const id = session.ids[session.idx], q = qById(id);
  if (isMulti(q)) {
    const a = normArr(session.answers[id]);
    const k = a.indexOf(i); if (k >= 0) a.splice(k,1); else a.push(i);
    session.answers[id] = a.sort((x,y)=>x-y); renderQuestion(); return;
  }
  session.answers[id] = i;
  if (session.idx < session.ids.length - 1) { session.idx++; renderQuestion(); }
  else renderQuestion();
};
window.setOpenText = function (txt) { const id=session.ids[session.idx]; session.answers[id]=Object.assign({}, session.answers[id]||{}, {text:txt}); };
window.markOpen = function (ok) { const id=session.ids[session.idx]; session.answers[id]=Object.assign({}, session.answers[id]||{}, {self:ok}); renderQuestion(); };
...
window.submitQuiz = function () {
  const left = session.ids.length - session.ids.filter(qid => isAnswered(session.answers[qid])).length;
  ...
  s.ids.forEach(id => {
    const q = qById(id);
    if (isCorrect(q, s.answers[id])) { correct++; score += qScore(q); } else wrongIds.push(id);
  });
  const db = load();
...
function renderResult(name, index) {
  ...
  const review = showIds.length ? showIds.map(id => {
    ...
    const opts = isOpen(q) ? `<div class="expl"><b>參考答案：</b>${correctLabel(q)}<br><b>你的作答：</b>${(my&&my.text)||answerLabel(q,my)}</div>` : q.o.map((txt, i) => {
      let cls = "opt";
      const ca = normArr(q.a), ma = normArr(my);
      if (ca.includes(i)) cls += " correct";
      else if (ma.includes(i)) cls += " wrong";
      const mk = ca.includes(i) ? "正解" : (ma.includes(i) ? "你選的" : "");
    ...
    return `<div class="review-item ${ok ? "ok" : "ng"}">
      <div class="qhead">
        ...
        <span class="chip unit">${q.u}</span>
        ${!isAnswered(my) ? '<span class="chip">未作答</span>' : `<span class="chip">你的答案：${answerLabel(q,my)}</span>`}
      </div>
      <div class="stem">${q.s}</div>
      ${figuresHTML(q)}
      <div class="opts">${opts}</div>
  ...
  app.innerHTML = `
    ...
    <div class="card center">
      ...
      <div class="score-big">${att.score}<span style="font-size:1.2rem;color:var(--muted)"> / ${att.ids.length===QUESTIONS.length ? (QUIZ_META.total || att.ids.reduce((sum,id)=>sum+qScore(qById(id)),0)) : att.ids.reduce((sum,id)=>sum+qScore(qById(id)),0)}</span></div>
...
function renderPractice() {
  ...
  const opts = isOpen(q) ? `<div class="expl"><b>非選擇題：</b>先作答，再點自評。</div>
    <textarea id="openAns" class="open-input" placeholder="可簡記自己的作答重點（只存在本機）" oninput="setOpenText(this.value)" ${revealed?'disabled':''}>${(my&&my.text)||""}</textarea>
    ${revealed ? "" : `<div class="row" style="margin-top:10px"><button class="btn primary" onclick="answerPractice(true)">我自評正確</button><button class="btn bad" onclick="answerPractice(false)">需要再練</button></div>`}` : q.o.map((txt, i) => {
    let cls = "opt"; const ca=normArr(q.a), ma=normArr(my);
    if (revealed) { if (ca.includes(i)) cls += " correct"; else if (ma.includes(i)) cls += " wrong"; }
    const mk = revealed ? (ca.includes(i) ? "正解" : (ma.includes(i) ? "你選的" : "")) : "";
  ...
  const feedback = revealed ? `
    <div class="card tight" style="border-color:${isCorrect(q,my) ? "var(--good)" : "var(--bad)"}">
      <div class="qhead"><span class="chip ${isCorrect(q,my) ? "good" : "bad"}">${isCorrect(q,my) ? "✓ 答對了！" : "✗ 答錯了，正解是 " + correctLabel(q)}</span></div>
      <div class="expl"><b>💡 詳解：</b>${q.e}</div>
    </div>` : "";
...
window.answerPractice = function (i) { if (session.revealed) return; const id=session.ids[session.idx], q=qById(id); if (isOpen(q)) session.answers[id]=Object.assign({}, session.answers[id]||{}, {self:i}); else if (isMulti(q)) { const a=normArr(session.answers[id]); const k=a.indexOf(i); if(k>=0)a.splice(k,1); else a.push(i); session.answers[id]=a.sort((x,y)=>x-y); renderPractice(); return; } else session.answers[id]=i; session.revealed = true; renderPractice(); };
...
window.finishPractice = function () {
  ...
  let score = 0; s.ids.forEach(id => { const q=qById(id); if (isCorrect(q, s.answers[id])) { correct++; score += qScore(q); } else wrongIds.push(id); });
  ...
  const attempt = {
    ...
    correct, total: s.ids.length, score: score, wrongIds,
    durationSec: Math.max(1, Math.round((Date.now() - s.start) / 1000))
  };
...
function renderPracticeSummary(name, att) {
  ...
  const wrong = att.wrongIds.map(id => {
    ...
    return `<div class="review-item ng">
      ...
      <div class="opts"><div class="opt correct"><span class="k">${correctLabel(q)}</span><span>官方參考答案</span><span class="mk">正解</span></div></div>
