...
function renderQuiz(viewOnly=false){let q=qById(sessionIds[idx]);let ans=state.answers[q.id];let chosen=norm(ans);$('#quiz').innerHTML=`<div class="quiz-head"><div><b>${idx+1}/${sessionIds.length}　${q.year} 第 ${q.q} 題</b><div class="muted">${q.domain}／${q.subdomain}｜${q.topic}</div></div><button class="ghost" onclick="$('#quizDialog').close();apply()">關閉</button></div><div class="quiz-body"><figure class="qfig"><img class="question-img zoomable" src="${q.img}" alt="${q.year} 第 ${q.q} 題單題截圖" onclick="openZoom('${q.img}','${q.year} 第 ${q.q} 題｜${q.topic.replace(/'/g,'’')}')"><figcaption>單題截圖：點擊圖片可放大觀看。${q.pageImg?`<button class="linkbtn" onclick="openZoom('${q.pageImg}','${q.year} 第 ${q.q} 題｜整頁來源')">看整頁來源</button>`:''}</figcaption></figure><p class="muted">請閱讀上方單題截圖作答；若題組前文不足，可點「看整頁來源」。</p>${answerUI(q,chosen,ans)}<div id="fb">${ans!==undefined?feedback(q,ans):''}</div><div class="nav"><button class="ghost" onclick="prevQ()">上一題</button><button class="ghost" onclick="nextQ()">下一題</button><button onclick="markReview('${q.id}')">標為待熟練</button></div></div>`;}
...
function openZoom(src,title){
  ...
  if(!d){
    d=document.createElement('dialog'); d.id='zoomDialog';
...
window.startOne=startOne;window.openSource=openSource;window.choose=choose;window.selfEval=selfEval;window.prevQ=prevQ;window.nextQ=nextQ;window.markReview=markReview;window.openZoom=openZoom;
fetch('questions.json?v=20260616b').then(r=>r.json()).then(j=>{DATA=j.questions;filtered=DATA.slice();fillFilters();apply();});
