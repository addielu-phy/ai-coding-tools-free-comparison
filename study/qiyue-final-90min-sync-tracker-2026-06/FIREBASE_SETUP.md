...
- 同步資料請只放：日期、科目、檢核項目、分鐘、簡短備註、週一～週日家長一句回報。不要填完整姓名、班級座號、私人題本照片或敏感個資。
...
- `LocalStorageAdapter`：預設啟用，不需要任何設定；資料只存在目前瀏覽器，也可先使用週一～週日家長一句回報模板。
- `FirestoreAdapter`：當 `firebase-config.js` 的 `enabled: true` 且 `projectId` 等 Web config 已填入時自動啟用；同一個 `?plan=` 會同步同一份檢核與週回報模板。
